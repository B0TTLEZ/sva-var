...
def insert_sva_into_module(
    ...
    """
    将单个SVA字符串插入到指定模块的endmodule之前（逻辑不变）
    """
    try:
...
def find_top_module_file(sv_file_paths: List[Path], top_module: str) -> Path:
    """
    从SV文件列表中找到包含顶层模块的文件（仅关键日志，无多余输出）
    """
    for sv_path in sv_file_paths:
        try:
            with open(sv_path, 'r', encoding='utf-8') as f:
                content = f.read()
            module_pattern = re.compile(
                rf"^\s*module\s+{re.escape(top_module)}\b",
                re.MULTILINE
            )
            if module_pattern.search(content):
                return sv_path
        except Exception as e:
            logging.warning(f"检查文件 {sv_path} 是否包含顶层模块失败: {e}")
    logging.warning(f"未找到包含顶层模块 {top_module} 的文件，使用第一个文件")
    return sv_file_paths[0]

def copy_sv_files_and_insert_sva(
    sv_file_paths: List[Path],
    output_dir: Path,
    top_module: str,
    sva_string: str
) -> List[Path]:
    """
    复制所有SV文件并插入SVA（仅保留关键日志，去掉多余的文件复制日志）
    """
    output_dir.mkdir(parents=True, exist_ok=True)
    output_sv_paths = []
    top_sv_path = find_top_module_file(sv_file_paths, top_module)
    
    for sv_path in sv_file_paths:
        output_sv_path = output_dir / sv_path.name
        output_sv_paths.append(output_sv_path)
        
        try:
            if sv_path == top_sv_path:
                # 仅打印和原始代码一致的"SVA植入完成"日志
                with open(sv_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                ...
                # 静默复制其他文件，不打印日志（和原始单文件版本一致）
                shutil.copy2(sv_path, output_sv_path)
        except Exception as e:
            logging.error(f"处理SV文件 {sv_path} 失败: {e}", exc_info=True)
            shutil.copy2(sv_path, output_sv_path)
    
    return output_sv_paths
...
def generate_tcl_script(
    ...
    """
    生成多文件版本的TCL脚本（日志输出和原始一致）
    """
    try:
        # 构建多文件analyze指令
        analyze_lines = ["analyze -sv12 \\"]
        for idx, sv_path in enumerate(sv_file_paths):
            sv_path_abs = sv_path.resolve()
            if idx == len(sv_file_paths) - 1:
                analyze_lines.append(f"    {sv_path_abs}")
            else:
                analyze_lines.append(f"    {sv_path_abs} \\")
        analyze_cmd = "\n".join(analyze_lines)
...
def run_jg_verification(
    ...
    """
    运行JG验证（恢复原始版本：不打印JG命令实时输出，只写日志）
    """
    try:
...
def process_single_assertion(
    ...
    try:
        ...
        result["paths"] = {
            "original_sv_files": [str(p) for p in sv_file_paths],
            "ft_dir": str(ft_dir),
            "tcl": str(tcl_path),
        ...
        output_sv_paths = copy_sv_files_and_insert_sva(
            sv_file_paths=sv_file_paths,
            output_dir=ft_dir,
            top_module=top_module,
            sva_string=assertion_text
        )
        if not output_sv_paths:
            result["error_message"] = "单个SVA植入失败"
            logging.error(result["error_message"])
            return result

        if not generate_tcl_script(tcl_path, top_module, clock_signal, reset_signal, output_sv_paths):
...
# ============================ 主函数（多文件+原始输出风格） ============================
def main():
    parser = argparse.ArgumentParser(description="验证断言正确性：针对多文件SV设计验证每个断言是否正确")
    parser.add_argument("--svfiles-loc", required=True, type=Path,
                        help="包含SV文件路径的txt文件（每行一个路径，如sv_files.txt）")
    ...
    if not args.svfiles_loc.exists():
        logging.error(f"SV文件列表文件不存在: {args.svfiles_loc}")
        sys.exit(1)
    try:
        with open(args.svfiles_loc, 'r', encoding='utf-8') as f:
            sv_file_paths = [
                Path(line.strip()).resolve() 
                for line in f.readlines() 
                if line.strip() and not line.strip().startswith("#")
            ]
        missing_files = [p for p in sv_file_paths if not p.exists()]
        if missing_files:
            logging.error(f"以下SV文件不存在: {', '.join(map(str, missing_files))}")
            sys.exit(1)
        if not sv_file_paths:
            logging.error("SV文件列表文件中无有效路径")
            sys.exit(1)
    except Exception as e:
        logging.error(f"读取SV文件列表失败: {e}", exc_info=True)
        sys.exit(1)
    ...
    logging.info(f"SV文件列表: {args.svfiles_loc.resolve()}")
    logging.info(f"包含的SV文件: {', '.join(map(str, sv_file_paths))}")
